...
//  IssueViewer
//
//  Created by carlos chaguendo on 23/08/17.
...
class WhatsappViewController: UIViewController, UITextFieldDelegate, CNContactViewControllerDelegate {
	...
	var previusColor: UIColor = UIColor.red
	...
	override func viewDidDisappear(_ animated: Bool) {
		...
		navigationController?.navigationBar.tintColor = previusColor
	}
	...
	@IBAction func sendMessageAction(_ sender: Any) {
		...
		if phoneText.text != nil && phoneText.text?.isEmpty == false {
			...
			if UIApplication.shared.canOpenURL(url) == false {
				alertInformation("Whatsapp", message: "It is not installed", handler: {
			...
			newContact.phoneNumbers.append(CNLabeledValue(label: "Whatsapp", value: CNPhoneNumber(stringValue: "3136524912")))
