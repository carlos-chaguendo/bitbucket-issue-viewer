...
public class HtmlParser {
    ...
    public static let h3Style = Style("h3")
        .font(.boldSystemFont(ofSize: 15))
        .foregroundColor(Colors.primary)
        .baselineOffset(5)

    public static let emStyle = Style("em")
    ...
    /// Bloque paragrafo
    public static let pStyle = Style("p").baselineOffset(5)
    ...
    public static let trStyle = Style("tr").underlineStyle(NSUnderlineStyle.single)
    ...
    public class func parse(html input: String, addFooter: Bool = false, includeAttach: Bool = true) -> NSAttributedString {
        ...
        while let olTag = html.substring(between: "<ol>", and: "</ol>") {
            var current = olTag
            let list = NSMutableString(string: String.newLine)
            var counter = 1
            while let litag = current.substring(between: "<li>", and: "</li>") {
                let out = litag.replacingOccurrences(of: "<li>", with: "\(counter).\t").replacingOccurrences(of: "</li>", with: String.empty)
                current = current.replacingOccurrences(of: litag, with: out)
                list.append(out)
                list.append(String.newLine)
                counter += 1
            }
            list.append(String.newLine)
            html = html.replacingOccurrences(of: olTag, with: list.description)
        }

        while let olTag = html.substring(between: "<ul>", and: "</ul>") {
            var current = olTag
            let list = NSMutableString(string: String.newLine)
            while let litag = current.substring(between: "<li>", and: "</li>") {
                let out = litag.replacingOccurrences(of: "<li>", with: "∙\t").replacingOccurrences(of: "</li>", with: String.empty)
                current = current.replacingOccurrences(of: litag, with: out)
                list.append(out)
                list.append(String.newLine)
            }
        ...
        var tags = [emStyle, strong, pStyle, final, h3Style, div, pre, del, blockquote, trStyle, code]
